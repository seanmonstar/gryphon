...
function parseRequest(req, opts) {
...
function authenticate(req, opts) {
  ...
  var timeSkew = opts.timeSkew || 60;
  if (Math.abs(now - header.ts) > (timeSkew * 1000)) {
    dbug('stale timestamp', header.ts);
    return null;
  }
  ...
  var ret = verified ? header.pubkey : null;
  dbug('verified? %s', ret);
  return ret;
}
